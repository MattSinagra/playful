  ...
  constructor(projectConfig, serverConfig, cwd, callbacks) {
    ...
    _defineProperty(this, "addHooks", compiler => {
      ...
      const {
        ...
        if (this.firstCompileCompleted) {
          ...
          const messages = (0, _formatWebpackMessages.default)(raw); // further remove the absolute path
    ...
    _defineProperty(this, "addTsHooks", (compiler, plugin) => {
      ...
      tsHooks.receive.tap('afterTypeScriptCheck', (diagnostics, lints) => {
        ...
        const format = message => `${(0, _typescriptFormatter.default)(message, true).split(this.cwd).join('.')}`;
        ...
        if (this.firstCompileCompleted) {
          ...
          try {
            const messages = await tsMessagesPromise; // add ts errors to stats so that it shows up in the client/browser

            stats.compilation.errors.push(...messages.errors);
            stats.compilation.warnings.push(...messages.warnings); // don't display the delayed message of "waiting for type result"
